...
def getXYcbow(sentences, window, word_dict, n):
    # generates the data points for the CBOW model
    X = []
    Y = []
...
def getXYskipGram(sentences, window, word_dict):
    # generates the data points for the Skip-gram model
    word_pairs = []
...
def trainModel(sentences, word_dict, window, d, epochs, cbow:True):
    """This method trains CBOW/Skip-gram algorithm based on the inputs
        ...
        cbow: wheater to train CBOW (True) or Skip-gram (False) model
    """
    n = len(word_dict)
    if cbow:
    ...
    # Plotting the training progress through epochs
    plt.figure()
